...
func TestGetShareValue(t *testing.T) {
	require.EqualValues(t, types.GetShareValue(math.NewInt(100), math.LegacyMustNewDecFromStr("0.9")), math.NewInt(90))
	require.EqualValues(t, types.GetShareValue(math.NewInt(100), math.LegacyMustNewDecFromStr("1.1")), math.NewInt(110))

	// truncated
	require.EqualValues(t, types.GetShareValue(math.NewInt(101), math.LegacyMustNewDecFromStr("0.9")), math.NewInt(90))
	require.EqualValues(t, types.GetShareValue(math.NewInt(101), math.LegacyMustNewDecFromStr("1.1")), math.NewInt(111))

	require.EqualValues(t, types.GetShareValue(math.NewInt(100), math.LegacyMustNewDecFromStr("0")), math.NewInt(0))
	require.EqualValues(t, types.GetShareValue(math.NewInt(0), math.LegacyMustNewDecFromStr("1.1")), math.NewInt(0))
}
...
func TestAddOrInit(t *testing.T) {
	...
	strIntMap.AddOrSet("a", math.NewInt(1))
	require.Equal(t, strIntMap["a"], math.NewInt(1))

	// Added when the key existed on the map
	strIntMap.AddOrSet("a", math.NewInt(1))
	require.Equal(t, strIntMap["a"], math.NewInt(2))
}
...
func TestSafeMath(t *testing.T) {
	maxInt, _ := math.NewIntFromString("115792089237316195423570985008687907853269984665640564039457584007913129639935")
	...
	for _, tc := range []struct {
		...
		},
		{
			func() {
				...
				math.LegacyNewDecFromBigIntWithPrec(i, sdk.Precision)
			},
			false,
		},
		{
			func() {
				i, _ := new(big.Int).SetString("133499189745056880149688856635597007162669032647290798121690100488888732861290034376435130433535", 10)
				d := math.LegacyNewDecFromBigIntWithPrec(i, sdk.Precision)
				d.Add(math.LegacyNewDecWithPrec(1, sdk.Precision))
			},
			true,
		},
		{
			func() {
				...
				d := math.LegacyNewDecFromBigIntWithPrec(i, sdk.Precision)
				d.Quo(math.LegacyNewDecWithPrec(1, 10))
			},
			true,
		},
	} {
