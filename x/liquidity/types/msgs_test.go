...
func TestMsgCreateRangedPool(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			"too large max price",
			func(msg *types.MsgCreateRangedPool) {
				msg.MaxPrice = math.LegacyNewDecFromInt(math.NewIntWithDecimal(1, 25))
			},
...
func TestMsgMMOrder(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			func(msg *types.MsgMMOrder) {
				msg.MaxSellPrice = math.LegacyZeroDec()
			},
		...
		},
		{
			...
			func(msg *types.MsgMMOrder) {
				msg.MinSellPrice = math.LegacyZeroDec()
			},
		...
		},
		{
			...
			func(msg *types.MsgMMOrder) {
				msg.MaxBuyPrice = math.LegacyZeroDec()
			},
		...
		},
		{
			...
			func(msg *types.MsgMMOrder) {
				msg.MinBuyPrice = math.LegacyZeroDec()
			},
