...
func OfferCoinAmount(dir OrderDirection, price math.LegacyDec, amt math.Int) math.Int {
	switch dir {
	case Buy:
...
func MatchableAmount(order Order, price math.LegacyDec) (matchableAmt math.Int) {
...
func TotalMatchableAmount(orders []Order, price math.LegacyDec) (amt math.Int) {
	amt = sdk.ZeroInt()
	for _, order := range orders {
...
func inv(x math.LegacyDec) (r math.LegacyDec) {
	r = oneDec.Quo(x)
	return
}
...
func poolOrderPriceGapRatio(poolPrice, currentPrice math.LegacyDec) (r math.LegacyDec) {
	if poolPrice.IsZero() {
		poolPrice = math.LegacyNewDecWithPrec(1, sdk.Precision) // lowest possible math.LegacyDec
	}
