...
func (ob *OrderBook) BuyOrdersAt(price math.LegacyDec) []Order {
	return ob.buys.ordersAt(price)
}

// SellOrdersAt returns sell orders at given price in the order book.
// Note that the orders are not sorted.
func (ob *OrderBook) SellOrdersAt(price math.LegacyDec) []Order {
	return ob.sells.ordersAt(price)
}

func (ob *OrderBook) HighestPrice() (math.LegacyDec, bool) {
...
func (ob *OrderBook) LowestPrice() (math.LegacyDec, bool) {
...
func (ob *OrderBook) stringRepresentation(prices []math.LegacyDec) string {
	if len(prices) == 0 {
		return "<nil>"
	}
...
func (ob *OrderBook) String() string {
	priceSet := map[string]math.LegacyDec{}
	for _, tick := range append(ob.buys.ticks, ob.sells.ticks...) {
		priceSet[tick.price.String()] = tick.price
	}
	prices := make([]math.LegacyDec, 0, len(priceSet))
...
func (ticks *orderBookTicks) findPrice(price math.LegacyDec) (i int, exact bool) {
...
func (ticks *orderBookTicks) ordersAt(price math.LegacyDec) []Order {
...
func (ticks *orderBookTicks) highestPrice() (math.LegacyDec, int, bool) {
	if len(ticks.ticks) == 0 {
		return math.LegacyDec{}, 0, false
	}
	if ticks.priceIncreasing {
...
func (ticks *orderBookTicks) lowestPrice() (math.LegacyDec, int, bool) {
	if len(ticks.ticks) == 0 {
		return math.LegacyDec{}, 0, false
	}
	if ticks.priceIncreasing {
